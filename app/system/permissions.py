...
async def request_permissions(missing_permissions: List[str]) -> bool:
    ...
    try:
        ...
        permission_messages = {
            "accessibility": "SelFlow needs Accessibility permission to monitor system events.",
            "full_disk_access": "SelFlow needs Full Disk Access to monitor file operations.",
            "screen_recording": "SelFlow needs Screen Recording permission for advanced monitoring.",
            "automation": "SelFlow needs Automation permission to interact with applications.",
        }

        for permission in missing_permissions:
            message = permission_messages.get(
                permission, f"SelFlow needs {permission} permission."
            )
...
4. Find SelFlow in the list and check the box next to it
5. If SelFlow is not in the list, click the "+" button and add it
        """.strip(),
        "full_disk_access": """
...
3. Find SelFlow in the list and expand it
4. Check the boxes for applications you want SelFlow to control
        """.strip(),
    }
