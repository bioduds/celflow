...
class PatternDetector(nn.Module):
    ...
    async def analyze(self, event_data: dict) -> Optional[Dict[str, Any]]:
        ...
        Returns pattern information if detected, None otherwise.
        """
        try:
            # Convert event data to feature vector
            ...
            # Extract results
            pattern_probs = outputs['pattern_probs'].squeeze(0)
            confidence = outputs['confidence'].squeeze(0).item()
            
            # Find most likely pattern
            max_prob_idx = torch.argmax(pattern_probs).item()
            max_prob = pattern_probs[max_prob_idx].item()
            
            # Only return if confident enough
            if confidence > 0.3 and max_prob > 0.4:
                pattern_type = self.pattern_types[max_prob_idx % len(self.pattern_types)]
                
                return {
                    'type': pattern_type,
                    'confidence': confidence,
                    'probability': max_prob,
                    'features': features[:10],  # First 10 features for debugging
                    'detector_id': id(self)
                }
